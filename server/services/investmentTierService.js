/**
 * Authoritative Investment Tier and Interest Calculation Service for Growvest.
 * 
 * ─── OFFICIAL INVESTMENT TIERS ──────────────────────────────────────────────
 * - 1–14 Days   → 6% p.a.  (Early duration rate for custom dates < 15 days)
 * - 15–29 Days  → 12% p.a. (15 Days Plan / Tier)
 * - 30–89 Days  → 15% p.a. (1 Month / 30 Days Plan / Tier)
 * - 90–179 Days → 18% p.a. (3 Months / 90 Days Plan / Tier)
 * - 180–364 Days→ 20% p.a. (6 Months / 180 Days Plan / Tier)
 * - 365+ Days   → 24% p.a. (1 Year / 365 Days Plan / Tier)
 * 
 * ─── RATE CONSTRAINT RULE ──────────────────────────────────────────────────
 * Applicable Tier Rate <= Selected Plan Tier Rate
 * A custom withdrawal date can qualify for a LOWER existing tier, but NEVER for
 * a higher tier than the selected plan.
 * 
 * ─── INTEREST FORMULA ──────────────────────────────────────────────────────
 * Annual simple interest prorated for actual eligible holding days:
 * Interest = (Principal × Annual Rate % × Eligible Days) / (100 × 365)
 */
...
/**
 * Authoritatively calculates investment tier, rate, duration, interest, and maturity values.
 */
function calculateInvestmentTier({
  planType,
  amount,
  ...
  // Find raw matching tier based on actual duration days
  ...
  if (isFullMaturity) {
    ...
    tierExplanation = 'Below the 15-day tier: qualifies for 6% p.a. early-duration rate';
  } else {
